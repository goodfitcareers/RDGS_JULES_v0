...
from sqlmodel import SQLModel, Session, create_engine, select # select for query construction

# Import models (ensure they are all imported if SQLModel.metadata.create_all is used elsewhere)
from backend.models import Client, Role, RoleStatus # Role, RoleStatus might be used later

# Import DTOs (schemas)
from backend.schemas import (
    ClientCreate, ClientRead,
    RoleCreate, RoleRead, RoleUpdate, RoleStatusUpdate, # For future role routes
...
@clients_router.post("/", response_model=ClientRead, status_code=status.HTTP_201_CREATED)
def create_client(
    ...
    db_client = Client.model_validate(client_data) # Pydantic v2 / SQLModel style for creating model from schema
    ...
    try:
        ...
        db_session.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error creating client: {str(e)}")
    return db_client
...
def update_client(
    ...
    client_update_data = client_data.model_dump(exclude_unset=True) # Get only fields that were set
    ...
    try:
        ...
        db_session.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error updating client: {str(e)}")
    return db_client
...
def delete_client(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error deleting client: {str(e)}")
