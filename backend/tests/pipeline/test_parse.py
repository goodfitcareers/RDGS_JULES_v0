...
from backend.pipeline import parse_roles_node, GraphState
from backend.settings import settings # To access settings.OPENAI_API_KEY if needed by underlying code
from openai import APITimeoutError, APIConnectionError, APIStatusError, RateLimitError, BadRequestError, OpenAI
...
def test_parse_roles_successful(mock_openai_class):
    ...
    mock_message.content = json.dumps({"roles": [{"company_name": "TestCo", "title": "Tester"}]})
    ...
    assert output_state['roles_draft'] == [{"company_name": "TestCo", "title": "Tester"}]
...
def test_parse_roles_missing_resume_text(mock_openai_class):
    ...
    assert "Cannot parse roles, resume text is missing." in output_state['current_node_error']
...
def test_parse_roles_llm_returns_malformed_json(mock_openai_class):
    ...
    mock_response = MagicMock()
    mock_message = MagicMock()
    mock_message.content = "{'company': 'TestCo', 'title': 'Tester'}" # Invalid JSON (single quotes)
    type(mock_response.choices[0]).message = PropertyMock(return_value=mock_message)
...
@patch('backend.pipeline.OpenAI')
def test_parse_roles_openai_generic_api_errors(mock_openai_class, error_type, error_message_prefix):
    """Test handling of various OpenAI API errors."""
    mock_openai_instance = MagicMock(spec=OpenAI)
    mock_chat_completions_instance = MagicMock()
    # Simulate the error being raised by the .create() call
    mock_chat_completions_instance.create = MagicMock(side_effect=error_type("Simulated API error"))
    mock_openai_instance.chat.completions = mock_chat_completions_instance
    mock_openai_class.return_value = mock_openai_instance
...
def test_parse_roles_openai_api_status_error(mock_openai_class):
    ...
    error_instance = APIStatusError("Simulated API Status error", response=mock_error_response, body=None)
    ...
    assert output_state['roles_draft'] == []
    assert output_state['current_node_error'] is not None
    assert "OpenAI API status error (code 400): Detailed error from API" in output_state['current_node_error']
...
def test_parse_roles_with_pre_existing_error(mock_openai_class):
    ...
    mock_response = MagicMock()
    mock_message = MagicMock()
    mock_message.content = json.dumps({"roles": [{"company_name": "TestCo", "title": "Developer"}]})
    type(mock_response.choices[0]).message = PropertyMock(return_value=mock_message)
    ...
    assert output_state_skip['roles_draft'] == [] # Should skip due to pre-existing error_message
    assert output_state_skip['current_node_error'] is None # No new error from this node as it skipped
    assert output_state_skip['error_message'] == "Error from extract_node" # Original error preserved
    ...
    initial_state_no_pre_error = get_initial_state(resume_text="Successful parse", error_message=None)
    output_state_no_pre_error = parse_roles_node(initial_state_no_pre_error)
    assert output_state_no_pre_error['roles_draft'] == [{"company_name": "TestCo", "title": "Developer"}]
    ...
    mock_chat_completions_instance.create.side_effect = APITimeoutError("Timeout during parse")
    ...
    output_state_pre_error_and_new_error = parse_roles_node(initial_state_pre_error_and_new_error)
    assert output_state_pre_error_and_new_error['roles_draft'] == []
    assert "Timeout during parse" in output_state_pre_error_and_new_error['current_node_error']
    assert "Previous Node Error; OpenAI API timeout" in output_state_pre_error_and_new_error['error_message']
...
def test_parse_roles_llm_returns_json_without_roles_key(mock_openai_class):
    ...
    mock_response = MagicMock()
    mock_message = MagicMock()
    mock_message.content = json.dumps({"data": [{"company_name": "TestCo"}]}) # "roles" key is missing
    type(mock_response.choices[0]).message = PropertyMock(return_value=mock_message)
...
def test_parse_roles_prompt_construction(mock_openai_class):
    ...
    assert resume_content in messages[0]["content"] # Check if resume text is in the prompt
    assert "JSON object containing a single key \"roles\"" in messages[0]["content"] # Check for key instructions
