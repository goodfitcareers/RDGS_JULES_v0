...
from unittest.mock import MagicMock, patch  # ANY is useful for verifying objects

# Assuming Role, RoleStatus are in backend.models
from sqlalchemy.exc import SQLAlchemyError

# parse_date_string for test data prep
...
def test_sync_roles_successful(mock_create_engine, mock_session_class):
    ...
    mock_session_class.return_value.__enter__.return_value = (
        mock_session_instance  # For "with Session(...) as session:"
    )
    ...
    roles_data = [
        ...
        },
        {
            ...
            "start_date": "Jul 2023",
            "end_date": "Present",
            "description_points": ["Led things."],
        },
    ]
    initial_state = get_initial_state(
        client_id=str(client_uuid), roles_draft=roles_data
    )
    ...
    assert output_state.get("synced_roles_count", 0) == len(
        roles_data
    )  # Check if this key is returned
...
def test_sync_roles_missing_client_id(mock_create_engine, mock_session_class):
    initial_state = get_initial_state(
        client_id=None, roles_draft=[{"company_name": "Test"}]
    )
    output_state = sync_roles_node(initial_state)
...
def test_sync_roles_invalid_client_id_format(mock_create_engine, mock_session_class):
    initial_state = get_initial_state(
        client_id="not-a-uuid", roles_draft=[{"company_name": "Test"}]
    )
    output_state = sync_roles_node(initial_state)
...
@patch("backend.pipeline.Session")
@patch("backend.pipeline.create_engine")
def test_sync_roles_missing_roles_draft_empty_list(
    mock_create_engine, mock_session_class
):
...
@patch("backend.pipeline.Session")
@patch("backend.pipeline.create_engine")
def test_sync_roles_role_data_missing_required_fields(
    mock_create_engine, mock_session_class
):
    mock_session_instance = MagicMock()
...
def test_sync_roles_db_commit_error(mock_create_engine, mock_session_class):
    ...
    roles_data = [
        {"company_name": "TestCo", "title": "Tester", "start_date": "2023-01-01"}
    ]
    initial_state = get_initial_state(roles_draft=roles_data)
    output_state = sync_roles_node(initial_state)
...
def test_sync_roles_db_add_error(mock_create_engine, mock_session_class):
    ...
    roles_data = [
        {"company_name": "FailCo", "title": "Bad Role"},
        {
            "company_name": "GoodCo",
            "title": "Good Role",
        },  # This won't be added if loop breaks
    ]
    initial_state = get_initial_state(roles_draft=roles_data)
    output_state = sync_roles_node(initial_state)
...
def test_sync_roles_with_pre_existing_error(mock_create_engine, mock_session_class):
    ...
    )  # This node itself did not produce an error
    assert (
        output_state["error_message"] == "Previous error from another node"
