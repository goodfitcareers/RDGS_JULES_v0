from datetime import date, datetime, timezone  # MODIFIED: Ensured timezone is imported
...
def test_evidencesnippet_model():
    ...
    # Test defaults
    es_default = EvidenceSnippet(
        role_id=role_uuid, snippet_text="Another important point."
    )
...
def test_pydantic_validation_for_new_models():
    ...
    with pytest.raises(ValidationError):  # Check for multiple missing fields
        SourceDocument.model_validate(
            {"client_id": client_uuid}
        )  # path, mime_type, checksum missing

    with pytest.raises(ValidationError):  # Check for multiple missing fields
        Role.model_validate(
            {"client_id": client_uuid}
        )  # company_name, title, output_text missing

    with pytest.raises(
        ValidationError, match="type=enum"
    ):  # Updated regex for enum validation error
        Role.model_validate(
            {
                "client_id": client_uuid,
                "company_name": "Test",
